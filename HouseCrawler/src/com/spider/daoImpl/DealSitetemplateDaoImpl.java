...
import com.spider.mapper.DealSitetempalteMapper;
...
//对新闻模板表的操作
public class DealSitetemplateDaoImpl implements SitetemplateDao {
	...
	public ArrayList<DealSitetemplate> getSiteByLanguageAndLocation(DatabaseStatus dbStatus, int language, int locations){
		String sql="";
		...
		ArrayList<DealSitetemplate> rs = new ArrayList();
		try {
			rs = (ArrayList<DealSitetemplate>) jdbcTemplate.query(sql,
					new DealSitetempalteMapper());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ArrayList<NewsSitetemplate> getAllAvailableTemplate() {
		...
		ArrayList<NewsSitetemplate> rs = (ArrayList<NewsSitetemplate>) jdbcTemplate
				.query(sql, new DealSitetempalteMapper());
		return rs;
	}
