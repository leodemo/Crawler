...
import com.spider.mapper.CommunitySitetempalteMapper;
...
//对新闻模板表的操作
public class CommunitySitetemplateDaoImpl implements SitetemplateDao {
	...
	public ArrayList<CommunitySitetemplate> getSiteByLanguageAndLocation(DatabaseStatus dbStatus, int language, int locations){
		String sql="";
		...
		ArrayList<CommunitySitetemplate> rs = new ArrayList();
		try {
			rs = (ArrayList<CommunitySitetemplate>) jdbcTemplate.query(sql,
					new CommunitySitetempalteMapper());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ArrayList<NewsSitetemplate> getAllAvailableTemplate() {
		String sql = "select * from " + tableName;
		ArrayList<NewsSitetemplate> rs = (ArrayList<NewsSitetemplate>) jdbcTemplate
				.query(sql, new CommunitySitetempalteMapper());
		return rs;
	}
