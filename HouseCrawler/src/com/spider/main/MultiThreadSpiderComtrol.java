...
import com.spider.daoImpl.HouseSitetemplateDaoImpl;
...
public class MultiThreadSpiderComtrol {
    private static Logger MainLogger =
            Logger.getLogger(MultiThreadSpiderComtrol.class);
    //	对本地数据库进行重连
    ...
    //	对远程数据库进行重连
    public static void remoteDBReconnect(ApplicationContext context, DataDao dataDao, SitetemplateDao newSistemplateDao){
        try {
            dataDao.getDataSource().getConnection().close();
            newSistemplateDao.getDataSource().getConnection().close();
            context = new ClassPathXmlApplicationContext("beans.xml");
            newSistemplateDao = (SitetemplateDao) context.getBean("siteDB");
    ...
    public static void main(String args[]) {
        ...
        ApplicationContext context = new ClassPathXmlApplicationContext(
                "beans.xml");
        SitetemplateDao sitetemplateDao = (HouseSitetemplateDaoImpl) context
                .getBean("siteDB");
        NewsConfig config = (NewsConfig) context.getBean("config");
        System.out.println(config.getLinuxPicPath());
        UrlDao urlDao = (UrlDao) context.getBean("newsurl");
        DataDao dataDao = (DataDao) context.getBean("news");
        ...
        while (true) {
            ...
            // 获取新闻模板
            ArrayList<NewsSitetemplate> siteList = (ArrayList<NewsSitetemplate>)sitetemplateDao
...
//            conSynThread.start();
            ...
            for (int i = 0; i < urlSpiderThreadNum; i++) {
                MultiUrlSpider urlSpider = new MultiUrlSpider(urlDao,contentCount, config,
                        i, urlSpiderThreadNum,
                        dbStatus);
                urlSpider.setNewsSiteTemplateList(siteList);
            ...
            for (int i = 0; i < contentSpiderThreadNum; i++) {
                ...
                contentSpider.setNewsSiteList(siteList);
...
//					dataDao, dataStatisticsDao, siteList, dbStatus);
//			statistics.run();
            // 等待下一轮
