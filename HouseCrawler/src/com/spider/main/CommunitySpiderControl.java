...
import com.spider.daoImpl.CommunityDaoImpl;
import com.spider.daoImpl.HouseDaoImpl;
import com.spider.daoImpl.CommunitySitetemplateDaoImpl;
...
public class CommunitySpiderControl {
    private static Logger MainLogger =
            Logger.getLogger(CommunitySpiderControl.class);
    //	对本地数据库进行重连
    ...
    public static void main(String args[]) {
        ...
        ApplicationContext context = new ClassPathXmlApplicationContext(
                "beans.xml");
        SitetemplateDao sitetemplateDao = (CommunitySitetemplateDaoImpl) context
                .getBean("communitySiteDB");
        NewsConfig config = (NewsConfig) context.getBean("config");
        System.out.println(config.getLinuxPicPath());
        UrlDao urlDao = (UrlDaoImpl) context.getBean("communityurl");
        DataDao dataDao = (CommunityDaoImpl) context.getBean("community");
        ...
        while (true) {
            ...
            // 获取新闻模板
            ArrayList<CommunitySitetemplate> siteList = (ArrayList<CommunitySitetemplate>)sitetemplateDao
...
//            conSynThread.start();
            ...
            for (int i = 0; i < urlSpiderThreadNum; i++) {
                MultiUrlSpider urlSpider = new MultiUrlSpider(urlDao,contentCount, config, i, urlSpiderThreadNum,
                        dbStatus);
                urlSpider.setCommunitySiteTemplateList(siteList);
            ...
            for (int i = 0; i < contentSpiderThreadNum; i++) {
                ...
                contentSpider.setCommunitySiteList(siteList);
